...
@section('title', 'Masuk Cabang Dinas')
@section('subtitle', 'Silakan masuk ke akun Cabang Dinas Anda.')
...
@endif
<form action="{{ route('cabang.login.post') }}" method="POST" class="auth-form">
    @csrf
    <div class="form-group">
        <label for="nik">NIK</label>
        <div class="input-with-icon">
            <i data-lucide="user" class="input-icon"></i>
            <input id="nik" type="text" name="nik" value="{{ old('nik') }}" placeholder="Masukkan NIK Cabang Dinas" class="form-input {{ $errors->has('nik') ? 'input-error' : '' }}" required maxlength="16" pattern="[0-9]{16}" oninput="this.value = this.value.replace(/[^0-9]/g, '')">
        </div>
    </div>
    <div class="form-group">
