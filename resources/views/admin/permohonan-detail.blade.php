...
@section('title', 'Detail Pengajuan #' . $permohonan->id_pengajuan . ' - SI JEBOL Admin')
...
<div class="page-header">
    <div class="header-content">
        ...
        <p class="header-subtitle">SI JEBOL / Verifikasi / #{{ $permohonan->id_pengajuan }}</p>
    </div>
...
<div class="main-grid">
    ...
    <div style="display: flex; flex-direction: column; gap: 24px;">
        ...
        @if(strtolower($permohonan->jenis_pengajuan) === 'sekolah' || ($permohonan->masyarakat && $permohonan->masyarakat->tipe_pendaftar === 'sekolah'))
            @php
            ...
            <div class="panel-box">
                ...
                <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 20px; padding: 16px; background: #f8fafc; border-radius: 12px;">
                    ...
                    <div>
                        <div style="font-weight: 800; font-size: 1.1rem; color: var(--text-main);">{{ strtoupper($namaSekolah) }}</div>
                        <div style="font-size: 0.85rem; color: var(--text-muted); font-weight: 600;">Jumlah Siswa Diajukan: {{ $jumlahAnak }} Orang</div>
                    </div>
                </div>
                ...
                <div class="info-list">
                    <div class="info-grid">
                        <div class="info-row">
                            ...
                            <span class="info-value">{{ $namaPic }}</span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">NIK Petugas</span>
                            <span class="info-value">{{ $nikPic }}</span>
                        </div>
                    </div>
                    <div class="info-grid">
                        <div class="info-row">
                            <span class="info-label">Nomor WhatsApp</span>
                            <span class="info-value">{{ $phonePic }}</span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">Email</span>
                            <span class="info-value">{{ $emailPic }}</span>
                        </div>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Alamat Sekolah</span>
                        <span class="info-value">{{ $alamatSekolah }}</span>
                    </div>
            ...
            <div class="panel-box">
                ...
                <div style="display: flex; align-items: center; gap: 16px; margin-bottom: 20px; padding: 16px; background: #f8fafc; border-radius: 12px;">
                    <div style="width: 48px; height: 48px; background: var(--primary); color: white; border-radius: 50%; display: grid; place-items: center; font-size: 1.2rem; font-weight: 800;">
                        {{ strtoupper(substr($permohonan->masyarakat->nama ?? 'W', 0, 1)) }}
                    </div>
                    <div>
                        <div style="font-weight: 800; font-size: 1.1rem; color: var(--text-main);">{{ strtoupper($permohonan->masyarakat->nama ?? 'Warga') }}</div>
                        <div style="font-size: 0.85rem; color: var(--text-muted); font-weight: 600;">NIK: {{ $permohonan->masyarakat->nik ?? $permohonan->nik ?? '-' }}</div>
                    </div>
                </div>
                ...
                <div class="info-list">
                    <div class="info-grid">
                        ...
                        <div class="info-row">
                            <span class="info-label">Email</span>
                            <span class="info-value">{{ $permohonan->masyarakat->email ?? '-' }}</span>
                        </div>
                    </div>
                    <div class="info-row">
                    ...
                    <div style="margin-top: 12px;">
                        ...
                        <div style="display: flex; flex-direction: column; gap: 12px;">
                            ...
                            @endif
                            @if(!$permohonan->file_ktp && !$permohonan->file_kk)
                            <div style="padding: 16px; background: #f8fafc; border-radius: 12px; text-align: center; color: var(--text-muted); font-size: 0.9rem; font-weight: 500;">
                ...
                <div class="info-list">
                    ...
                    <div class="info-grid" style="margin-top: 12px;">
                        <div class="info-row">
                            ...
                            <span class="info-value" style="font-size: 1.1rem; font-weight: 800; color: var(--primary);">{{ $permohonan->jumlah_orang ?? '-' }} Orang</span>
                        </div>
                        <div class="info-row">
                            <span class="info-label">Jumlah Petugas</span>
                            <span class="info-value" style="font-size: 1.1rem; font-weight: 800; color: var(--primary);">{{ $permohonan->jumlah_petugas ? $permohonan->jumlah_petugas . ' Orang' : 'Belum Ditentukan' }}</span>
                        </div>
                    </div>
                </div>
            @else
                ...
                <div style="margin-bottom: 24px;">
                    ...
                    <div class="info-list">
                        ...
                        <div class="info-grid">
                            <div class="info-row"><span class="info-label">Tipe Smartphone</span><span class="info-value">{{ $permohonan->tipe_smartphone ?? 'Android / iPhone' }}</span></div>
                            <div class="info-row"><span class="info-label">Jumlah Orang</span><span class="info-value">{{ $permohonan->jumlah_orang ?? '-' }} Orang</span></div>
                        </div>
                        <div class="info-row">
                            <span class="info-label">Status Aktivasi</span>
                            <span style="font-weight: 700; color: var(--primary);">{{ $permohonan->status_ikd ?? 'Belum Aktivasi' }}</span>
                        </div>
                    </div>
                </div>
                @endif
                ...
                <div style="margin-bottom: 24px; border-top: 1px solid #e2e8f0; padding-top: 24px;">
                    ...
                    <div class="info-list">
                        <div class="info-row"><span class="info-label">Jenis Pengajuan</span><span class="info-value">{{ $permohonan->jenis_pengajuan ?? 'Baru / Hilang / Rusak' }}</span></div>
                        <div class="info-row"><span class="info-label">Keterangan</span><span class="info-value">{{ $permohonan->keterangan_ktp ?? '-' }}</span></div>
                        <div class="info-row">
                ...
                <div style="margin-top: 24px; padding-top: 20px; border-top: 2px dashed #e2e8f0;">
                    <div class="info-grid">
                        <div class="info-row"><span class="info-label">Jumlah Petugas</span><span class="info-value">{{ $permohonan->jumlah_petugas ? $permohonan->jumlah_petugas . ' Orang' : 'Belum Ditentukan' }}</span></div>
                        <div class="info-row"><span class="info-label">Total Peserta</span><span class="info-value">{{ $permohonan->jumlah_orang ?? '-' }} Orang</span></div>
                    </div>
                </div>
            @endif
        </div>
    ...
    <div style="display: flex; flex-direction: column; gap: 24px;">
            ...
            <div class="info-list">
                <div class="info-row">
                    ...
                    <span class="info-value" style="color: var(--primary); font-size: 1.1rem; font-weight: 800;">
                        {{ $permohonan->nomor_tiket ?? ('#JB-' . str_pad($permohonan->id_pengajuan, 4, '0', STR_PAD_LEFT)) }}
                    </span>
                </div>
                <div class="info-grid">
                    <div class="info-row">
                        <span class="info-label">Tanggal Pengajuan</span>
                        <span class="info-value">{{ $permohonan->tanggal_pengajuan ? $permohonan->tanggal_pengajuan->translatedFormat('d F Y') : '-' }}</span>
                    </div>
                    <div class="info-row">
                        <span class="info-label">Status</span>
                        <div>
                            <span class="status-badge {{ $st['class'] }}">
                                <i data-lucide="{{ $st['icon'] }}" style="width: 14px;"></i> {{ $st['label'] }}
                            </span>
                        </div>
                    </div>
                </div>
                <div class="info-row">
                    ...
                    <span class="info-value">{{ $permohonan->jenis_layanan }}</span>
                </div>
                <div class="info-grid">
                    <div class="info-row">
                        <span class="info-label">Lokasi JEBOL</span>
                        <span class="info-value">{{ $permohonan->lokasi->nama_lokasi ?? $permohonan->lokasi_pelayanan ?? '-' }}</span>
                    </div>
                    <div class="info-row">
        ...
        <div class="side-by-side-grid">
            ...
            <div class="panel-box no-print" style="background: #f8fafc; height: 100%;">
                ...
                @if($permohonan->keterangan && $permohonan->status != 'ditolak' && $permohonan->status != 'menunggu_verifikasi' && $permohonan->status != 'pending')
                <div style="background: white; border: 1px solid #bfdbfe; padding: 16px; border-radius: 12px; margin-bottom: 24px;">
                    ...
                    <div style="font-style: italic; font-weight: 500; color: #1e3a8a;">"{{ $permohonan->keterangan }}"</div>
                </div>
                @endif

                @if(in_array(trim(strtolower(auth()->user()->role ?? '')), ['admin', 'admin pusat', 'cabang', 'cabang_dinas', 'petugas', 'petugas cabang', 'petugas kecamatan']))
                
                    <form id="statusForm" action="{{ route('admin.permohonan.status', $permohonan->id_pengajuan) }}" method="POST">
                        @csrf
                    ...
                    <div style="display: flex; flex-direction: column; gap: 4px;">
                        ...
                        {{-- VERIFIKASI FLOW (SIDE-BY-SIDE + NOTES) --}}
                        @if($permohonan->status == 'menunggu_verifikasi' || $permohonan->status == 'pending')
                        <div style="margin-bottom: 20px;">
                            <div style="background: #f1f5f9; border: 1px solid #cbd5e1; padding: 16px; border-radius: 12px; margin-bottom: 16px;">
                                ...
                                <div style="font-size: 0.9rem; font-weight: 600; color: #334155;">{{ $permohonan->keterangan ?? '-' }}</div>
                            </div>
                            ...
                            <div style="margin-bottom: 20px;">
                                ...
                                <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
                                    <span style="background: #cbd5e1; border: 1px solid #94a3b8; color: #1e293b; padding: 6px 12px; border-radius: 8px; font-size: 0.75rem; font-weight: 700; text-transform: uppercase;">{{ $permohonan->status == 'pending' ? 'Pending' : 'Menunggu Verifikasi' }}</span>
                        ...
                        {{-- TOLAK (STANDALONE FOR STATUS !== MENUNGGU_VERIFIKASI) --}}
                        @if($permohonan->status == 'terverifikasi')
                        ...
                        {{-- JADWALKAN (KHUSUS SEKOLAH) --}}
                        @if($permohonan->status == 'terverifikasi' && strtolower($permohonan->jenis_pengajuan) === 'sekolah')
                        ...
                        <div style="background: white; border: 1px solid #e2e8f0; padding: 16px; border-radius: 12px; text-align: center; color: var(--text-muted); font-weight: 600; font-size: 0.9rem;">
                            Pengajuan ini telah ditutup ({{ strtoupper($permohonan->status) }}).
                        </div>
                        @endif
                    </div>
                @endif
            ...
            <div class="panel-box" style="height: 100%;">
                ...
                @if($permohonan->status == 'ditolak')
                    <div style="background: #fef2f2; border: 1px solid #fecaca; padding: 20px; border-radius: 12px; text-align: center;">
                        ...
                        <p style="font-size: 0.9rem; color: #dc2626; margin: 0;">{{ $permohonan->tanggal_pengajuan ? $permohonan->tanggal_pengajuan->translatedFormat('d F Y') : '-' }}</p>
                ...
                @else
                    <div style="padding-top: 12px;">
                        @foreach($allStatus as $idx => $s)
                        @php
                            $sOrder    = $statusOrder[$s['key']] ?? 0;
                            $isDone    = $sOrder <= $currentOrder;
                            $isCurrent = $s['key'] == $permohonan->status;
                        @endphp
                        <div class="timeline-item {{ $isDone ? 'done' : '' }} {{ $isCurrent ? 'active' : '' }}">
                            <div class="timeline-line"></div>
                            <div class="timeline-dot">
                                <i data-lucide="{{ $s['icon'] }}" style="width: 12px; height: 12px;"></i>
                            </div>
                            <div class="timeline-content">
                                <h4 class="timeline-title" style="color: {{ $isCurrent ? 'var(--primary)' : ($isDone ? 'var(--text-main)' : 'var(--text-muted)') }}">{{ $s['label'] }}</h4>
    ...
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            <div class="form-group">
                ...
                <input type="text" id="inputJadwalLokasi" value="{{ $permohonan->lokasi->nama_lokasi ?? $permohonan->lokasi_pelayanan ?? $permohonan->masyarakat->alamat }}" required class="form-control">
            </div>
...
<div id="completionModal" class="modal-backdrop">
    <div class="modal-content">
        ...
        <div class="modal-body">
            ...
            @if(strtolower($permohonan->jenis_pengajuan) === 'sekolah')
                <div style="background: #f8fafc; padding: 16px; border-radius: 12px; margin-bottom: 20px; display: flex; flex-direction: column; gap: 16px;">
                    @if($isKtp)
                    <div>
                        <label style="font-size: 0.8rem; font-weight: 700; color: var(--primary);">KTP-el (Target: {{ $permohonan->jumlah_orang }} Orang)</label>
                        <input type="number" id="inputSudahKtp" value="{{ $permohonan->jumlah_orang }}" min="0" max="{{ $permohonan->jumlah_orang }}" class="form-control" style="margin-top: 8px;">
                    </div>
                    @endif
                    @if($isIkd)
                    <div>
                        <label style="font-size: 0.8rem; font-weight: 700; color: var(--primary);">IKD (Target: {{ $permohonan->jumlah_orang }} Orang)</label>
                        <input type="number" id="inputSudahIkd" value="{{ $permohonan->jumlah_orang }}" min="0" max="{{ $permohonan->jumlah_orang }}" class="form-control" style="margin-top: 8px;">
                    </div>
                    @endif
                    @if($isKia)
                    <div>
                        <label style="font-size: 0.8rem; font-weight: 700; color: var(--primary);">KIA (Target: {{ $permohonan->jumlah_orang }} Orang)</label>
                        <input type="number" id="inputSudahKia" value="{{ $permohonan->jumlah_orang }}" min="0" max="{{ $permohonan->jumlah_orang }}" class="form-control" style="margin-top: 8px;">
                    </div>
                    @endif
                </div>
            @else
                ...
                <input type="hidden" id="inputSudahKtp" value="{{ $isKtp ? 1 : 0 }}">
                <input type="hidden" id="inputSudahIkd" value="{{ $isIkd ? 1 : 0 }}">
                <input type="hidden" id="inputSudahKia" value="{{ $isKia ? 1 : 0 }}">
            @endif
        ...
        <div class="modal-body">
            <div class="form-group">
                ...
                <div style="display: flex; flex-direction: column; gap: 8px; margin-top: 8px;">
                    @foreach(['Dokumen Tidak Terbaca','Data NIK Tidak Valid','Dokumen Kadaluarsa','Data Tidak Lengkap'] as $reason)
                    <label style="display: flex; align-items: center; gap: 10px; padding: 10px 16px; border: 1px solid #e2e8f0; border-radius: 10px; cursor: pointer;">
                        <input type="radio" name="reason_category" value="{{ $reason }}" style="width: 16px; height: 16px;">
                        <span style="font-weight: 500; font-size: 0.95rem;">{{ $reason }}</span>
                    </label>
                    @endforeach
                </div>
            </div>
...
<script>
    ...
    @endif
    @if(session('error'))
        Swal.fire({ icon:'error', title:'Gagal!', text:'{{ session("error") }}', confirmButtonColor:'#003178' });
    @endif
</script>
@endsection
