...
@section('title', 'Penilaian Layanan - SI JEBOL Admin')
...
<style>
    ...
    @media (max-width: 768px) {
        .page-header { flex-direction: column; text-align: center; padding: 32px 24px; gap: 24px; }
...
<div class="dashboard-grid">
    ...
    <div class="stat-card">
        ...
        <div>
            ...
            <div class="stat-value">{{ number_format($avgKeseluruhan, 2, ',', '.') }} <span style="font-size: 1rem; color: var(--text-muted);">/ 5</span></div>
    ...
    <div class="stat-card">
        ...
        <div>
            ...
            <div class="stat-value" style="font-size: 1.4rem;">{{ $layananTerbanyakNama }}</div>
...
<div class="panel-box">
    <div class="filter-container">
        <form action="{{ route('admin.kepuasan.index') }}" method="GET" class="filter-grid">
            <div class="form-group">
                <label class="form-label">Cari Nama Pemohon</label>
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Masukkan nama..." class="form-control">
            </div>
            ...
            <div style="display: flex; gap: 8px;">
                ...
                <a href="{{ route('admin.kepuasan.laporan') }}" class="btn btn-outline">
        ...
        <div style="overflow-x: auto;">
            <table class="data-table">
                ...
                <tbody>
                    ...
                    <tr class="table-row" 
                        @click="currentId = '{{ $review->id_kepuasan }}'; currentMasyarakat = '{{ addslashes($review->masyarakat ? ($review->masyarakat->name ?? $review->masyarakat->nama) : 'Anonim') }}'; currentLayanan = '{{ addslashes($review->status_layanan ?? '-') }}'; currentKeseluruhan = {{ (int)round($review->nilai_kepuasan) }}; currentSaran = '{{ addslashes(str_replace(["\r", "\n"], ' ', $review->kritik_saran ?? '-')) }}'; currentDate = '{{ $review->tanggal_input ? $review->tanggal_input->format('d M Y H:i') . ' WIB' : '-' }}'; currentFoto = '{{ $review->foto_path ? asset('storage/' . $review->foto_path) : '' }}';"
                        :class="currentId === '{{ $review->id_kepuasan }}' ? 'active' : ''">
                        <td style="font-weight: 600; color: var(--text-muted);">{{ ($reviews->currentPage() - 1) * $reviews->perPage() + $loop->iteration }}</td>
                        <td style="font-weight: 600; color: var(--text-main);">{{ $review->masyarakat ? ($review->masyarakat->name ?? $review->masyarakat->nama) : 'Anonim' }}</td>
                        <td><span style="background: #f1f5f9; padding: 4px 10px; border-radius: 20px; font-size: 0.8rem; font-weight: 600;">{{ $review->status_layanan ?? '-' }}</span></td>
                        <td>
                            <div class="star-rating" title="{{ $review->nilai_kepuasan }}/5">
                                @for($i = 1; $i <= 5; $i++)
                        ...
                        <td>{{ $review->tanggal_input ? $review->tanggal_input->format('d/m/Y') : '-' }}</td>
