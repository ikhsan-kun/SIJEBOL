<?php $__env->startSection('title', 'Monitoring Pengajuan Sekolah'); ?>
...
<div class="stats-container">
    <div class="stat-card">
        ...
        <div class="stat-content">
            ...
            <p class="stat-value"><?php echo e(number_format($stats['total'] ?? 0)); ?></p>
    ...
    <div class="stat-card">
        ...
        <div class="stat-content">
            ...
            <p class="stat-value"><?php echo e(number_format($stats['pending'] ?? 0)); ?></p>
    ...
    <div class="stat-card">
        ...
        <div class="stat-content">
            ...
            <p class="stat-value"><?php echo e(number_format($stats['diproses'] ?? 0)); ?></p>
    ...
    <div class="stat-card">
        ...
        <div class="stat-content">
            ...
            <p class="stat-value"><?php echo e(number_format($stats['selesai'] ?? 0)); ?></p>
...
<div class="filter-card">
    <form class="filter-form" action="<?php echo e(route('cabang.monitoring')); ?>" method="GET" id="filter-form">
        <div class="filter-group" style="flex: 2;">
            <label class="filter-label">Pencarian</label>
            <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Cari tiket, sekolah, atau layanan..." class="filter-input">
        </div>
        <div class="filter-group">
            ...
            <select name="status" class="filter-input" onchange="document.getElementById('filter-form').submit()">
                ...
                <option value="menunggu_verifikasi" <?php echo e(request('status') === 'menunggu_verifikasi' ? 'selected' : ''); ?>>Menunggu</option>
        ...
        <div class="filter-group" style="flex: 1.5;">
            ...
            <div class="date-range">
                <input type="date" name="start_date" value="<?php echo e(request('start_date')); ?>" class="filter-input">
                <span style="color: var(--text-muted);">-</span>
                <input type="date" name="end_date" value="<?php echo e(request('end_date')); ?>" class="filter-input">
            </div>
        </div>
        <div style="display: flex; gap: 12px;">
            ...
            <?php if(request()->anyFilled(['search', 'status', 'start_date', 'end_date'])): ?>
            <a href="<?php echo e(route('cabang.monitoring')); ?>" class="btn-reset">
...
<div class="table-card">
    <div class="table-responsive">
        <table class="data-table">
            ...
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $permohonans; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $p): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                <?php
                ...
                <tr>
                    <td style="font-weight: 700; color: var(--primary);">
                        <a href="javascript:void(0)" onclick="openDetail(<?php echo e($p->id_pengajuan); ?>)" style="color: var(--primary); text-decoration: none;" onmouseover="this.style.textDecoration='underline'" onmouseout="this.style.textDecoration='none'">
                            #<?php echo e($p->nomor_tiket); ?>
                    ...
                    <td>
                        <div style="display: flex; align-items: center; gap: 8px;">
                            ...
                            <span style="font-weight: 600;"><?php echo e($school); ?></span>
                        </div>
                    </td>
                    <td>
                        <span style="background: #f8fafc; padding: 4px 8px; border-radius: 6px; font-size: 0.8rem; font-weight: 600; border: 1px solid #e2e8f0;">
                            <?php echo e($p->jenis_layanan ?? '—'); ?>

                        </span>
                    </td>
                    <td>
                        <div style="font-weight: 700;"><?php echo e(number_format($realisasi)); ?> <span style="color: var(--text-muted); font-weight: 500; font-size: 0.8rem;">/ <?php echo e(number_format($target)); ?></span></div>
                    </td>
                    <td>
                        <div style="font-weight: 600;"><?php echo e($p->tanggal_pengajuan->format('d M Y')); ?></div>
                        <div style="font-size: 0.75rem; color: var(--text-muted);"><?php echo e($p->tanggal_pengajuan->format('H:i')); ?> WIB</div>
                    </td>
                    <td>
                        <?php if($p->tanggal_kedatangan): ?>
                        <div style="font-weight: 600; color: var(--primary);"><?php echo e(\Carbon\Carbon::parse($p->tanggal_kedatangan)->format('d M Y')); ?></div>
                        <?php else: ?>
                        <span style="color: var(--text-muted); font-size: 0.85rem; font-style: italic;">Belum dijadwal</span>
                        <?php endif; ?>
                    </td>
                    <td>
                        <span class="status-badge <?php echo e($statusCls); ?>">
                            <i data-lucide="<?php echo e($statusIcon); ?>" style="width: 14px; height: 14px;"></i> <?php echo e($statusLabel); ?>

                        </span>
                    </td>
                    <td>
                        <button class="btn-detail" onclick="openDetail(<?php echo e($p->id_pengajuan); ?>)">
    ...
    <?php if(isset($permohonans) && method_exists($permohonans, 'hasPages') && $permohonans->hasPages()): ?>
    <div class="pagination-wrapper">
        <div style="font-size: 0.85rem; color: var(--text-muted); font-weight: 600;">
            Menampilkan <?php echo e($permohonans->firstItem() ?? 0); ?> - <?php echo e($permohonans->lastItem() ?? 0); ?> dari <?php echo e($permohonans->total() ?? 0); ?>
...
<script>
    const pengajuanData = {
        ...
        ?>
        <?php echo e($p->id_pengajuan); ?>: {
            tiket:       "#<?php echo e($p->nomor_tiket); ?>",
            school:      <?php echo json_encode($school, 15, 512) ?>,
            layanan:     <?php echo json_encode($p->jenis_layanan ?? '—', 15, 512) ?>,
            peserta:     "<?php echo e($peserta > 0 ? $peserta . ' Orang' : '—'); ?>",
            tglPengajuan:<?php echo json_encode($tglPengajuan, 15, 512) ?>,
            tglPelayanan:<?php echo json_encode($tglPelayanan, 15, 512) ?>,
            lokasi:      <?php echo json_encode($lokasi, 15, 512) ?>,
            statusLabel: <?php echo json_encode($statusLabel, 15, 512) ?>,
            statusCls:   <?php echo json_encode($statusCls, 15, 512) ?>,
            statusIcon:  <?php echo json_encode($statusIcon, 15, 512) ?>,
            keterangan:  <?php echo json_encode($keterangan, 15, 512) ?>,
            tags:        [<?php echo json_encode($ikd, 15, 512) ?>, <?php echo json_encode($kia, 15, 512) ?>, <?php echo json_encode($realisasi, 15, 512) ?>].filter(Boolean),
            picNama:     <?php echo json_encode($picNama, 15, 512) ?>,
            picPhone:    <?php echo json_encode($picPhone, 15, 512) ?>,
            picNik:      <?php echo json_encode($picNik, 15, 512) ?>,
            fileSuratPengantar: <?php echo json_encode($fileSuratPengantar, 15, 512) ?>,
            jumlahPetugas: <?php echo json_encode($jumlahPetugas, 15, 512) ?>,
            jadwalWaktu: <?php echo json_encode($jadwalWaktu, 15, 512) ?>,
            statusRaw:   <?php echo json_encode($p->status, 15, 512) ?>
        },
...
<?php echo $__env->make('layouts.panel', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\laragon\www\jeboll\resources\views/cabang/monitoring.blade.php ENDPATH**/ ?>