...
<body>
    ...
    <div class="jbl-186">
        ...
        <div class="stats-summary">
            <div class="stat-card-glass">
                <div class="stat-value">
                    <?php echo e(number_format($rataRata, 1)); ?> <i data-lucide="star" width="32" height="32" fill="#FFC107" color="#FFC107"></i>
                </div>
                <div class="stat-label">Rata-rata Penilaian</div>
            </div>
            <div class="stat-card-glass">
                <div class="stat-value"><?php echo e($persentasePuas); ?>%</div>
                <div class="stat-label">Tingkat Kepuasan</div>
            </div>
            <div class="stat-card-glass">
                <div class="stat-value"><?php echo e(number_format($totalUlasan)); ?></div>
        ...
        <div class="filter-section">
            ...
            <form action="<?php echo e(route('ulasan')); ?>" method="GET" style="margin: 0;">
                <select name="filter" class="sort-select" onchange="this.form.submit()">
                    <option value="all" <?php echo e(request('filter', 'all') == 'all' ? 'selected' : ''); ?>>Semua Bintang</option>
                    <option value="5-bintang" <?php echo e(request('filter') == '5-bintang' ? 'selected' : ''); ?>>5 ⭐⭐⭐⭐⭐</option>
                    <option value="4-bintang" <?php echo e(request('filter') == '4-bintang' ? 'selected' : ''); ?>>4 ⭐⭐⭐⭐</option>
                    <option value="3-bintang" <?php echo e(request('filter') == '3-bintang' ? 'selected' : ''); ?>>3 ⭐⭐⭐</option>
                    <option value="2-bintang" <?php echo e(request('filter') == '2-bintang' ? 'selected' : ''); ?>>2 ⭐⭐</option>
                    <option value="1-bintang" <?php echo e(request('filter') == '1-bintang' ? 'selected' : ''); ?>>1 ⭐</option>
                </select>
            </form>
        </div>
        ...
        <div style="margin-bottom: 40px; display: grid; grid-template-columns: repeat(auto-fill, minmax(350px, 1fr)); gap: 24px;">
                <?php $__empty_1 = true; $__currentLoopData = $reviews; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $review): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    ...
                    <div class="history-header">
                        <span class="history-date"><?php echo e($review->tanggal_input ? $review->tanggal_input->format('d M Y, H:i') : '-'); ?> WIB</span>
                        <?php
                            $statusClass = ($review->status ?? '') === 'followed_up' ? 'status-followed-up' : 'status-received';
                        ?>
                        <span class="status-badge <?php echo e($statusClass); ?>">
                            <?php if(($review->status ?? '') === 'followed_up'): ?>
                    ...
                    <div class="history-rating">
                        <?php for($i = 1; $i <= 5; $i++): ?>
                            <?php if($i <= $review->nilai_kepuasan): ?>
                    ...
                    <p class="history-feedback jbl-1360">"<?php echo e($review->kritik_saran ?? 'Tidak ada komentar.'); ?>"</p>
                    <?php if($review->foto_path): ?>
                        <div style="margin-top: 12px;">
                            <a href="javascript:void(0)" onclick="openImageModal('<?php echo e(Storage::url($review->foto_path)); ?>')" title="Klik untuk memperbesar">
                                <img src="<?php echo e(Storage::url($review->foto_path)); ?>" alt="Foto" style="max-width: 100%; max-height: 250px; border-radius: 8px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); transition: transform 0.2s;" onmouseover="this.style.transform='scale(1.02)'" onmouseout="this.style.transform='scale(1)'" />
                            </a>
                        </div>
                    <?php endif; ?>
                    <?php if($review->status === 'followed_up' && $review->admin_response): ?>
                        <div class="history-response">
                            <i data-lucide="message-square" width="16" height="16"></i>
                            <span><b>Balasan Admin:</b> <?php echo e($review->admin_response); ?></span>
                        </div>
                    <?php endif; ?>
                    <div style="display: flex; align-items: center; gap: 12px; margin-top: auto; padding-top: 16px; border-top: 1px solid #e2e8f0;">
                        <div style="width: 40px; height: 40px; border-radius: 50%; background: var(--primary); color: white; display: flex; align-items: center; justify-content: center; font-weight: bold; flex-shrink: 0;">
                            <?php echo e($review->masyarakat ? strtoupper(substr($review->masyarakat->name ?? $review->masyarakat->nama ?? 'A', 0, 1)) : 'A'); ?>

                        </div>
                        <div>
                            <div style="font-weight: 700; color: var(--primary); font-size: 0.95rem;"><?php echo e($review->masyarakat ? ($review->masyarakat->name ?? $review->masyarakat->nama) : 'Anonim'); ?></div>
                            <div style="font-size: 0.8rem; color: #64748b; display: flex; align-items: center; gap: 4px;"><i data-lucide="map-pin" width="12"></i> <?php echo e($review->masyarakat ? ($review->masyarakat->kecamatan ?? 'Kota Tegal') : 'Masyarakat'); ?></div>
                        </div>
                    </div>
                </div>
        ...
        <div class="cta-bottom">
            ...
            <a href="<?php echo e(route('register')); ?>" class="btn-cta">
...
<?php /**PATH D:\laragon\www\jeboll\resources\views/pengunjung/ulasan.blade.php ENDPATH**/ ?>